...
class ConvLoss:
    def __init__(self, kernel_size = 5, l1_loss=torch.nn.L1Loss):
        self.loss = l1_loss()
        self.fnc = torch.nn.functional
        self.ks = kernel_size

    def __call__(self, output, target):
        kernel = torch.ones(1, 1, self.ks, self.ks ).cuda()
        out = torch.unsqueeze(output.reshape(-1, 20, 20), 1)
        trg = torch.unsqueeze(target.reshape(-1, 20, 20), 1)
        out_kernel = self.fnc.conv2d(out, kernel)
        trg_kernel = self.fnc.conv2d(trg, kernel)
        new_size = out.shape[3]-self.ks+1
        out_kernel = torch.squeeze(out_kernel,1).reshape(-1, new_size*new_size)
        trg_kernel = torch.squeeze(trg_kernel,1).reshape(-1, new_size*new_size)
        loss = self.loss(out_kernel,trg_kernel)
        return loss
...
def _parse_args():
    ...
    # add by me
    args = parser.parse_args(['./dataset/','--dataset', 'carla','--train-towns', '3','--val-towns', '3',\
       '--train-weathers', '0',  '--val-weathers' ,'0', \
       '--model', 'interfuser_baseline', '--sched', 'cosine', '--epochs' ,'25', '--warmup-epochs' ,'5' ,'--lr' ,'0.0005', \
         '--batch-size', '4' , '-j', '1', '--no-prefetcher', '--eval-metric' ,'l1_error', \
            '--opt', 'adamw' ,'--opt-eps' ,'1e-8', '--weight-decay' ,'0.05' , \
               ...
               '--experiment', 'interfuser_baseline','--pretrained'])   #,'--log-wandb'
...
def main():
    ...
    if args.log_wandb:
        if has_wandb:
            wandb.init(project=args.experiment, config=args,  entity="marslab")
        else:
            _logger.warning(
    ...
    linear_scaled_lr = (
        args.lr * args.batch_size * 1 #torch.distributed.get_world_size()
        / 512.0
    ...
    train_loss_fns = {
        ...
        "cls": cls_loss,
        "stop_cls": cls_loss,
        "htmap":ConvLoss(5, l1_loss=l1_loss),
    }
    validate_loss_fns = {
        "traffic": MVTL1Loss(1.0, l1_loss=l1_loss),
        "waypoints": WaypointL1Loss(l1_loss=l1_loss),
...
def train_one_epoch(
    ...
    losses_htmap = AverageMeter()  #TODO add the other places
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if args.distributed:
                ...
                reduced_loss_velocity = loss_velocity.data if loss_velocity else 0*loss.data
